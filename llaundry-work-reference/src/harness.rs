//! Execution- and review-layer operations, extracted from linka's ops
...
//! the composed linka crate (`Store`, `Vcs`, core model types, and the
//! former `linka_review`/`orka` crate paths). The plan is to
//! re-home it behind a task-store trait with an adapter for linka.

use linka_review::CandidateStore as _;
use linka_review::FsCandidateStore;
use orka::AttemptStore as _;
use orka::FsAttemptStore;
...
fn validate_execution(
    ...
    let expected = format!("linka/candidates/{}", execution.attempt_id);
...
pub fn prepare_execution(
    ...
    let candidate_branch = format!("linka/candidates/{attempt_id}");
    let workbench = store
        .workbench_root()
        .canonicalize()
        .with_context(|| format!("resolving workbench {}", store.workbench_root().display()))?;
    let path = workbench.join(".orkatrees").join(&attempt_id);
    ...
    if materialize {
        ...
        vcs.commit_store(
            &store.store_name(),
            &format!("linka: begin attempt {attempt_id}"),
        )?;
        ...
        vcs.commit_store(
            &store.store_name(),
            &format!("linka: prepare attempt {attempt_id}"),
        )?;
    }
...
pub fn finalize_execution_attempt(
    ...
    vcs.commit_store(
        &store.store_name(),
        &format!("linka: finalize attempt {attempt_id}"),
    )?;
...
pub fn finish_attempt_workspace(
    ...
    if !orka::should_remove_workspace(
        keep,
...
pub fn amend_worker(
    ...
    store.write_result(id, &result, &notes)?;
    vcs.commit_store(&store.store_name(), &format!("linka: worked by {id}"))?;
    Ok(true)
}
...
pub fn create_review_for_attempt(store: &Store, vcs: &dyn Vcs, attempt_id: &str) -> Result<String> {
    ...
    vcs.commit_store(
        &store.store_name(),
        &format!("linka: review {implementation}"),
    )?;
    Ok(id)
}
...
fn review_workspace_location(store: &Store, id: &str) -> Result<(String, std::path::PathBuf)> {
    let branch = format!("linka/reviews/{id}");
    let workbench = store
        .workbench_root()
        .canonicalize()
        .with_context(|| format!("resolving workbench {}", store.workbench_root().display()))?;
...
pub fn decide_review(
    ...
    if decision == ReviewDecision::Accepted {
        ...
        vcs.commit_store(
            &store.store_name(),
            &format!("linka: prepare publication {id}"),
        )?;
        return recover_publication(store, vcs, id);
    }
    ...
    vcs.commit_store(
        &store.store_name(),
        &format!("linka: {:?} review {id}", decision),
    )
}
...
fn close_review(
    ...
    store.write_result(id, &result, notes)?;
    reviews(store).record_decision(&linka_review::Decision {
        candidate: id.into(),
        kind: decision,
        notes: notes.into(),
...
pub fn recover_publication(store: &Store, vcs: &dyn Vcs, id: &str) -> Result<()> {
    ...
    if target_now == publication.target_previous {
        ...
        match linka_review::publish_exact(
            &VcsPublisher { vcs }, &candidate, &publication.target, &expected,
        ) {
            Ok(()) => {}
            Err(linka_review::PublishError::NotFastForward) => bail!("reviewed candidate cannot fast-forward `{}`; create follow-up implementation work and review the changed result", publication.target),
            Err(linka_review::PublishError::Backend(error)) => return Err(error),
        }
    ...
    vcs.commit_store(
        &store.store_name(),
        &format!("linka: complete publication {id}"),
    )
}
...
impl linka_review::Publisher for VcsPublisher<'_> {
    type Error = anyhow::Error;
    fn publish(
        &self,
        candidate: &Candidate,
        target: &str,
        expected_previous: &linka_core::ArtifactRef,
    ) -> Result<bool> {
        self.vcs
...
pub fn node_state(store: &Store, id: &str) -> NodeState {
    linka_review::node_state(&ReviewAdapter { store }, id, current_status(store, id))
}
...
impl linka_review::ReviewStateView for ReviewAdapter<'_> {
    type Error = anyhow::Error;
...
pub fn recover_attempt(store: &Store, vcs: &dyn Vcs, attempt_id: &str) -> Result<Option<String>> {
    ...
    if !attempt.prepared {
        ...
        vcs.commit_store(
            &store.store_name(),
            &format!("linka: recover attempt {attempt_id}"),
        )?;
    }
