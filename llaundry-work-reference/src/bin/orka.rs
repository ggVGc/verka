//! `orka` — the driver for doing work on a linka node with an LLM.
//!
...
//! tool grant: the `linka` MCP server for graph operations, plus the built-in
...
//! to just the linka server (`--strict-mcp-config`).
//!
...
use orka::backend::{Backend, ClaudeCode, McpServer, OpenAiCodex, Session};

use linka::{ops, title_of, Author, Config, GitVcs, NodeMeta, Store, WorkedBy};

#[derive(Parser)]
#[command(
    name = "orka",
    version,
    about = "Run an LLM against a linka node"
)]
struct Cli {
    ...
    /// Path to the `linka-mcp` executable the model is allowed to use.
    /// Config `work.mcp-bin`; built-in default `linka-mcp`.
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let attempt_store = orka::FsAttemptStore::new(store.root());
    ...
    if model.is_none() {
        eprintln!(
            "orka: warning: {} did not report which model ran; worked_by records none",
            backend.name()
        );
    }
    ...
    if let Some(review) = review {
        eprintln!("orka: awaiting human review in node {review}");
    }

    if !ops::finish_attempt_workspace(&store, &vcs, &run_id, cli.keep_worktree)? {
        eprintln!(
            "orka: retained candidate worktree {} on {}",
            workspace.path.display(),
...
/// `linka ready --for llm` shows. A human-assigned node is waiting for a
...
/// must follow — graph changes through the `linka` MCP tools, files through
...
fn build_prompt(
    ...
    let mut p = vec![
        "You are an autonomous worker on a linka node graph.".to_string(),
        "Every graph change goes through the `linka` MCP tools. For real work you".to_string(),
...
mod tests {
    ...
    fn first_ready_skips_human_assigned_and_blocked_nodes() {
        let dir = std::env::temp_dir().join(format!("linka-next-test-{}", std::process::id()));
    ...
    fn observed_reads_mines_project_read_calls_from_the_transcript() {
        let dir = std::env::temp_dir().join(format!("orka-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let store = Store::init(dir.join(".linka")).unwrap();
        ...
        let log = [
            ...
            read(format!("{}/.linka/nodes/n/node.toml", workbench.display())),
            ...
            r#"{"type":"assistant","message":{"content":[{"type":"tool_use","name":"mcp__linka__show_node","input":{"id":"n"}}]}}"#.to_string(),
    ...
    fn prompt_states_the_node_and_the_tools_only_rule() {
        ...
        assert!(prompt.contains("linka` MCP tools"));
