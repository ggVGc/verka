...
//! the linka library, replacing the direct `linka_core` paths below.
...
use linka::store::blob_id;
use linka::{ArtifactRef, Author, DefinitionVersion, ProducerEvidence, ResultMeta as ResultRecord, ResultVersion};
use serde::{Deserialize, Serialize};
use std::path::{Path, PathBuf};

/// Producer evidence namespace owned by this application.
pub const EVIDENCE_NAMESPACE: &str = "orka";
...
mod tests {
    ...
    fn attempt_records_live_only_in_the_execution_namespace() {
        let root = std::env::temp_dir().join(format!("orka-store-{}", std::process::id()));
    ...
    fn git_workspace_manager_prepares_and_removes_isolated_branch() {
        let root = std::env::temp_dir().join(format!("orka-git-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
