...
impl Vcs for GitVcs {
    ...
    fn publish_fast_forward(&self, target: &str, expected: &str, new: &str) -> Result<bool> {
        ...
        let ancestor = git(
            &self.project,
            &["merge-base", "--is-ancestor", expected, new],
        )?;
        ...
        if checked_out.as_deref() == Some(target) {
            ...
            if checked(&self.project, &["rev-parse", "HEAD"])? != expected {
                return Ok(false);
            }
            checked(&self.project, &["merge", "--ff-only", new])?;
        } else {
            checked(&self.project, &["update-ref", target, new, expected])?;
        }
        Ok(true)
    }
}
...
mod store_mutation_tests {
    ...
    fn clean_store_mutation_is_one_commit_and_finishes_clean() {
        ...
        let id = ops::add(&store, &vcs, new_node("atomic addition"), None).unwrap();
    ...
    fn dirty_store_refuses_mutation_before_any_edit() {
        let (_temp, store, vcs) = workbench();
        let id = ops::add(&store, &vcs, new_node("existing"), None).unwrap();
        let before = checked(&store.workbench_root(), &["rev-parse", "HEAD"]).unwrap();
        std::fs::write(store.node_dir(&id).join("description.md"), "hand edit").unwrap();

        let error = ops::add(&store, &vcs, new_node("must not be created"), None).unwrap_err();
        ...
        assert_eq!(store.node_ids().unwrap(), vec![id]);
    }
    ...
    fn ignored_store_files_are_included_in_the_mutation_commit() {
        ...
        let id = ops::add(&store, &vcs, new_node("must be committed"), None).unwrap();
    ...
    fn preexisting_ignored_store_content_blocks_mutation() {
        ...
        assert!(store.node_ids().unwrap().is_empty());
    }
    ...
    fn concurrent_store_mutation_is_refused_until_lock_release() {
        ...
        let error = ops::add(&store, &vcs, new_node("contended"), None).unwrap_err();
        assert!(error.to_string().contains("mutation is in progress"));
        drop(lock);

        ops::add(&store, &vcs, new_node("after release"), None).unwrap();
    }
}
