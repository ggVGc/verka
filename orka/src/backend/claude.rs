...
/// Backend that shells out to Claude Code (`claude -p`): the linka MCP for
...
mod tests {
    ...
    fn sample_session() -> Session {
        Session {
            ...
            mcp: McpServer {
                name: "linka".into(),
                command: "linka-mcp".into(),
                args: vec!["--store".into(), "/wb/.linka".into()],
            },
        }
    }
    ...
    fn claude_command_grants_scoped_file_tools_but_no_shell() {
        ...
        assert_eq!(cfg["mcpServers"]["linka"]["command"], "linka-mcp");
        assert_eq!(cfg["mcpServers"]["linka"]["args"][1], "/wb/.linka");
    }
