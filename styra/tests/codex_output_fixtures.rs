...
use styra::event::{decode_line, Protocol, StyraEvent};
...
fn real_codex_exec_output_decodes_to_the_expected_event_sequence() {
    let events: Vec<StyraEvent> = EXEC_COMMAND
        .lines()
    ...
    assert!(
        !events.iter().any(|e| matches!(e, StyraEvent::Malformed { .. } | StyraEvent::Unknown { .. })),
    ...
    assert!(matches!(events[0], StyraEvent::ThreadStarted { .. }));
    assert_eq!(events[1], StyraEvent::TurnStarted);
    assert_eq!(
        events[2],
    ...
    assert_eq!(events[4], StyraEvent::AgentMessage { text: "done".into() });
    match &events[5] {
