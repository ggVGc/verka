...
//! extracted from the former llaundry-core crate.
//!
//! PARKED: not expected to compile yet. `FsGraphStore` no longer exists — the
//! graph store folded back into the llaundry library. This protocol is the
//! embryo of the planned task-store trait: llaundry-work should define the
//! trait and implement it with an adapter for llaundry (in-process or over
//! this wire format).
