...
| `--overlay-writes` | Turn all writable host bind mounts into overlays: readable host content, writable in the sandbox, discarded instead of reaching the host. This includes the default current-directory workspace, template and project-configured mounts, and `--write` mounts. |
